...
function GetUser(message, arg){
  var errormessage = "<:fcross:1064768376829919264> I failed finding that User...";
...
function GetRole(message, arg){
  var errormessage = "<:fcross:1064768376829919264> I failed finding that Role...";
  return new Promise(async (resolve, reject) => {
    var args = arg, client = message.client;
    if(!client || !message) return reject("CLIENT IS NOT DEFINED")
    if(!args || args == null || args == undefined) args = message.content.trim().split(/ +/).slice(1);
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if(!user && args[0] && args[0].length == 18) {
      ...
      for(const g of guilds){
        var members = g.members.cache.map(this_Code_is_by_KuaterCraft_0062 => this_Code_is_by_KuaterCraft_0062);
...
async function edit_Roster_msg(client, guild, the_roster_db) {
  try{
    ...
    let rosterembed = new Discord.MessageEmbed()
      .setColor(0x2b2d31).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es))
      ...
      if (data.rosterstyle == "1") {
        ...
        for (let i = 0; i < memberarray.length; i += 20) {
          ...
          if (!the_roster_db?.get(guild.id, "showallroles") || memberarray.length < 20)
            try { 
              rosterembed.addField(`**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_KuaterCraft_0062 => this_Code_is_by_KuaterCraft_0062).length}]__**`, role.members.map(this_Code_is_by_KuaterCraft_0062 => this_Code_is_by_KuaterCraft_0062).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substr(0, leftnum <= 1024 ? leftnum : 1024) + `${role.members.map(this_Code_is_by_KuaterCraft_0062 => this_Code_is_by_KuaterCraft_0062).length - 20 > 0 ? `\n${the_roster_db?.get(guild.id, "rosteremoji")} ***\`${role.members.map(this_Code_is_by_KuaterCraft_0062 => this_Code_is_by_KuaterCraft_0062).length - 20}\` other Members have this Role ...***`: ""}`.substr(0, 1024), the_roster_db?.get(guild.id, "inline"))
              break;
            } catch (e) {
            ...
            try {
              rosterembed.addField(i < 20 ? `**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_KuaterCraft_0062 => this_Code_is_by_KuaterCraft_0062).length}]__**` : `\u200b`, role.members.map(this_Code_is_by_KuaterCraft_0062 => this_Code_is_by_KuaterCraft_0062).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substr(0, leftnum <= 1024 ? leftnum : 1024), the_roster_db?.get(guild.id, "inline"))
            } catch (e) {
              console.log(e.stack ? String(e.stack).grey : String(e).grey)
            }
        }
        ...
        await msgs.forEach(async msg => {
            ...
            if(msg.type == "DEFAULT"){
              ...
              if (msg.reactions && msg.reactions.cache.size > 0){
                ...
                for(const reaction of msg.reactions.cache.map(this_Code_is_by_KuaterCraft_0062 => this_Code_is_by_KuaterCraft_0062)){                      
...
function stations(client, prefix, message) {
  ...
  try {
    const reyfm_iloveradio_embed = new MessageEmbed().setColor(0x2b2d31).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es)).setTitle("Pick your Station, by typing in the right `INDEX` Number!").setDescription(`Example: \`${prefix}radio 11\``);
    const stationsembed = new MessageEmbed().setColor(0x2b2d31).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es)).setTitle("Pick your Station, by typing in the right `INDEX` Number!").setDescription(`Example: \`${prefix}radio 44\``);
    const stationsembed2 = new MessageEmbed().setColor(0x2b2d31).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es)).setTitle("Pick your Station, by typing in the right `INDEX` Number!").setDescription(`Example: \`${prefix}radio 69\``);
    const stationsembed3 = new MessageEmbed().setColor(0x2b2d31).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es)).setTitle("Pick your Station, by typing in the right `INDEX` Number!").setDescription(`Example: \`${prefix}radio 120\``);
    const stationsembed4 = new MessageEmbed().setColor(0x2b2d31).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es)).setTitle("CUSTOM REQUESTS | Pick your Station, by typing in the right `INDEX` Number!");
    ...
    for (let i = 0; i < radios.OTHERS.request.length; i++) {
      ...
      if(requests.length > 1900){
        embeds.push(new MessageEmbed().setColor(0x2b2d31).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null).setFooter(client.getFooter(es)).setTitle("CUSTOM REQUESTS | Pick your Station, by typing in the right `INDEX` Number!").setDescription(`${requests}`))
        requests = "";
      }
    }
 ...
 */
  ...
  collector.on('collect', async b => {
      if(b?.user.id !== message.author.id)
        return b?.reply({content: `<:fcross:1064768376829919264> **Only the one who typed ${prefix}help is allowed to react!**`, ephemeral: true})
        //page forward
        if(b?.customId == "1") {
  ...
  for (let i = 0; i < guilds.length; i++) {
      try {
          ...
          await guild.channels.cache.filter(ch => ch.type == "GUILD_VOICE" && jointocreate.includes(ch.id)).each(async (channel, j) => {
              try{
                  let members = channel.members.map(this_Code_is_by_KuaterCraft_0062 => this_Code_is_by_KuaterCraft_0062);
          ...
          await guild.channels.cache.filter(ch => ch.type == "GUILD_VOICE").each(async vc => {
              try{
                  if(client.jointocreatemap.get(`tempvoicechannel_${vc.guild.id}_${vc.id}`) == vc.id){
                      let members = vc.members.map(this_Code_is_by_KuaterCraft_0062 => this_Code_is_by_KuaterCraft_0062);
...
async function create_transcript(message, client, msglimit) {
  ...
  let msgs = messageCollection.map(this_Code_is_by_KuaterCraft_0062 => this_Code_is_by_KuaterCraft_0062).reverse(); //reverse the array to have it listed like the discord chat
