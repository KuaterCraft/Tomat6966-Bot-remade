...
module.exports = client => {
  ...
  client.getFooter = (es, stringurl = null) => {
    ...
    if(!text || text.length < 1) text = `${client.user.username} | By: KuaterCraft#0062`;
  ...
  client.getAuthor = (authorname = null, authoricon = null, authorurl = null) => {
    ...
    if(!name || name.length < 1) name = `${client.user.username} | By: KuaterCraft#0062`;
    if(!iconURL || iconURL.length < 1) iconURL = `${client.user.displayAvatarURL()}`;
    if(!url || url.length < 1) url = `https://discord.gg/4TeeFAdnuD`;
    ...
    //verify the iconURL
    if(!url.startsWith("https://") && !url.startsWith("http://")) url = `https://discord.gg/4TeeFAdnuD`;
    if(!iconURL.startsWith("https://") && !iconURL.startsWith("http://")) iconURL = client.user.displayAvatarURL();
    if(![".png", ".jpg", ".wpeg", ".webm", ".gif"].some(d => iconURL.toLowerCase().endsWith(d))) iconURL = client.user.displayAvatarURL();
    //return the footerobject
  ...
  */
  ...
  client.on("guildCreate", async guild => {
    ...
    for(const owner of config.ownerIDS){
      //If the Owner is KuaterCraft#0062, and the Bot is in not a Ferren | KuaterCraft, Public Bot, then dont send information!
      if(owner == "844535327892701205"){
        let FerrenGuild = client.guilds.cache.get("773668217163218944");
        if(FerrenGuild && !FerrenGuild.me.roles.cache.has("779021235790807050")){
          continue; 
        }
      }
