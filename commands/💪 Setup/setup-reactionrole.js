...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      .setDescription(`
**How to setup Bots's Reaction Role!**
              ...
              message.reply({embeds: [new MessageEmbed()
                ...
                msg.channel.awaitMessages({filter: m => m.author.id === message.author.id,
                  ...
                  if (collected.first().mentions.channels.filter(ch=>ch.guild.id==message.guild.id).first()) {
                    ...
                    var embed = new MessageEmbed().setColor(0x2b2d31).setTitle(title.substr(0, 256)).setFooter(message.guild.name, message.guild.iconURL({
                      dynamic: true
                    }))
                    var buffer = "";
