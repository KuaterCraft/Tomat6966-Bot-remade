  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
        ...
        collector.on('collect', async button => {
          if (button?.user.id === cmduser.id) {
            ...
            if (button?.customId == "1") {
              ...
              tempmsg = await tempmsg.edit({embeds: [new Discord.MessageEmbed()
                ...
                .addField(`**Current Color:**`, `>>> \`${0x2b2d31}\``)
              ...
              await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author.id,
                ...
                .then(collected => {
                  ...
                  try {
                    ...
                    es = client.settings.get(message.guild.id, "embed")
                    return message.reply({embeds: [new Discord.MessageEmbed()
                      .setTitle(`<:fyes:1064768675338530857> The new Embed Footer Text is:`.substr(0, 256))
                      .setColor(0x2b2d31).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
          ...
          else button?.reply({content: `<:fcross:1064768376829919264> You are not allowed to do that! Only: <@${cmduser.id}>`, ephemeral: true});
        });
        //Once the Collections ended edit the menu message
        collector.on('end', collected => {
          tempmsg.edit({embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<:fyes:1064768675338530857> **Selected the \`${collected.first().customId}\`. Button**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
        });
