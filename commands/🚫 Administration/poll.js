...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      if (!args[0])
      {
        message.reply({embeds:  [new MessageEmbed().setColor(0x2b2d31).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
        .setFooter(client.getFooter(es))
        ...
        .setDescription(eval(client.la[ls]["cmds"]["administration"]["poll"]["variable4"]))
          ...
          msg.channel.awaitMessages({filter: m=>m.author.id === cmduser.id, max: 1, time: 30000, errors: ["time"]}).then(collected => {
            let channel = collected.first().mentions.channels.filter(ch=>ch.guild.id==message.guild.id).first();
              ...
              message.channel.send({embeds :[new MessageEmbed().setColor(0x2b2d31).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                .setFooter(client.getFooter(es))
