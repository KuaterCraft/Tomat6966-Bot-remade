...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      if (args[0]) {
        const embed = new MessageEmbed().setColor(0x2b2d31).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null);
...
Utility
Report`)
.addField("<:frefresh:1064768395112882177> **__STATS:__**",
 ...
 **\`${Math.floor(client.ws.ping)}ms\` Ping**`)
        ...
        function allotherembeds_eachcategory(filterdisabled = false){
          ...
          var embed0 = new MessageEmbed()
            ...
            .addField(` **Games Related Commands**`,  ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "games").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
          ...
          var embed1 = new MessageEmbed()
            ...
            .addField("\u200b", "__**Sub-Categorized Commands:**__")
            .addField(`**Mini Game to earn 💸**`,  ">>> " + client.commands.filter((cmd) => cmd.category === "💸 Economy" && cmd.type === "game").sort((a,b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
