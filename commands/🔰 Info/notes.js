...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      var button_disable = new MessageButton().setStyle('SECONDARY').setCustomId('notes_disable').setLabel('Stop the Buttons').setEmoji("1064768376829919264");
      
      var embeds = [];
      var currentPage = 0;
      if(!notes || notes.length == 0){
          embeds.push(new MessageEmbed().setColor(0x2b2d31)
          .setFooter(message.author.tag+ ` Page | 0/0`, message.author.displayAvatarURL({dynamic: true}))
          .setTitle(`<:fcross:1064768376829919264> No Notes created yet`)
      ...
      try{
        if(b?.user.id !== message.author.id)
          ...
          if(b?.customId == "notes_disable") {
            ...
            var buttonRow2 = new MessageActionRow().addComponents([button_create, button_edit, button_Delete, button_disable])
            allbuttons = [buttonRow1, buttonRow2]
            await b?.reply("<:fcross:1064768376829919264> **Disabled the Buttons**", true);
          ...
          if(b?.customId == "notes_jump") {
            ...
            await mmmmm.channel.awaitMessages({filter: m=>m.author.id == cmduser.id, max: 1, time: 180e3}).then(async collected => {
              ...
              if(Page < 0 || Page > embeds.length - 1){
                return message.reply("<:fcross:1064768376829919264> **Value out of Range!**").then(msg=>{
                  setTimeout(()=>{
                    try{msg.delete()}catch(e){console.log(String(e).grey)}
                  }, 2500)
                }).catch(e=>{
            ...
            msgtodelete.push(mmmm)
              ...
              await mmmm.channel.awaitMessages({filter: m=>m.author.id == cmduser.id, max: 1, time: 180e3}).then(async collected => {
                ...
                await mmmmm.channel.awaitMessages({filter: m=>m.author.id == cmduser.id, max: 1, time: 180e3}).then(async collected => {
                  ...
                  notes = client.notes.get(message.author.id, "notes");
                  embeds[0] = new MessageEmbed().setColor(0x2b2d31)
                  .setFooter(message.author.tag, message.author.displayAvatarURL({dynamic: true}))
                  .setTitle(`All of your Notes you can jump to!`)
                  .setDescription(`${notes.map((data, index) => `**\`Page: ${index + 2}/${notes.length + 1}\`:** ${String(data.title).substr(0, 80)}`).join("\n")}`.substr(0, 2048));
                  embeds[thenote + 1] = new MessageEmbed().setColor(0x2b2d31)
