...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {   
      ...
      //create the EMBED
      const embeduserinfo = new MessageEmbed().setColor(0x2b2d31).setTitle(`Banner of ${user.tag}`)
      let banner = false;
      try{
      ...
      if(banner) {
        ...
        embeduserinfo.setColor(user.hexAccentColor ? user.hexAccentColor : 0x2b2d31)
        embeduserinfo.setDescription(`[Download the Banner of **${user.tag}**](${banner}) <@${user.id}>`)
      } else {
        embeduserinfo.setColor(user.hexAccentColor ? user.hexAccentColor : 0x2b2d31)
        embeduserinfo.setTitle(`<:fcross:1064768376829919264> **Has no Banner!**`)
      }
