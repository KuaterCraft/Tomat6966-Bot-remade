  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      async function first_layer(){
        ...
        .setColor(0x2b2d31)
        .setAuthor('Owner Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/prohibited_1f6ab?.png',  'https://discord.gg/4TeeFAdnuD')
        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content : `<:fcross:1064768376829919264> You already selected something, this Selection is now disabled!`, ephermal : true});
              menuselection(menu);
            }
            else menu?.reply({content : `<:fcross:1064768376829919264> You are not allowed to do that! Only: <@${cmduser.id}>`, ephermal : true});
          }
        });
      }
            ...
            await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author.id,
              ...
              .then(collected => {
                ...
                if (user) {
                  ...
                  if(user.id == "844535327892701205"){
                    return message.channel.send({embeds: [new MessageEmbed()
                      .setFooter(client.getFooter(es))
                      .setColor(es.wrongcolor)
                      .setTitle(`You can't remove KuaterCraft#0062!`)
